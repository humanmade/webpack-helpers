...
If your build includes two configurations generated from `presets.production()` which output to the same folder, and one of these builds already includes a manifest using the name `production-asset-manifest.json`, that file may be overwritten on build by the new manifest generated by the other configuration. This is an edge case that would impact few projects, if any, but it does constitute a potential break.
