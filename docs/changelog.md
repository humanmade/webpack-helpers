...
- **Potentially Breaking**: A `production-asset-manifest.json` is now output on all production preset builds. [#153](https://github.com/humanmade/webpack-helpers/pull/153)

If your build includes two or more configurations generated from `presets.production()` which output to the same folder, one of those configurations uses no manifest, and another of those configurations already generates a manifest using the name `production-asset-manifest.json`, that file may be overwritten on build by the new manifest generated by the other configuration. This is an edge case that should impact few projects, if any, but it does constitute a potential break if you already use that specific manifest name in a multi-configuration setup.
